...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const map: Record<string, string> = {
    CONFIRMED: 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-800',
    CANCELLED:  'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/40 dark:text-rose-300 dark:border-rose-800',
    PENDING:    'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/40 dark:text-amber-300 dark:border-amber-800',
...
const DisplayField: React.FC<{ icon?: React.ReactNode; value: string }> = ({ icon, value }) => (
  <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-xs text-gray-700 font-medium flex items-center gap-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300">
...
export const UserProfilePage: React.FC = () => {
  ...
  const inputCls = 'w-full bg-white border border-gray-300 focus:border-[#00a8cc] focus:outline-none text-gray-900 text-xs rounded-lg px-3 py-2.5 transition-colors dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100';

  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-24 dark:bg-gray-950 dark:text-gray-100">

      {/* Page Header */}
      <div className="border-b border-gray-200 bg-white py-8 dark:border-gray-800 dark:bg-gray-900">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative group shrink-0">
              <div className="w-14 h-14 rounded-full overflow-hidden border border-gray-200 bg-gray-100 flex items-center justify-center dark:border-gray-700 dark:bg-gray-800">
                {user.avatarUrl
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeTab === 'profile' && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 sm:p-8 space-y-6 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center justify-between border-b border-gray-200 pb-4 dark:border-gray-700">
                <div>
                  <h2 className="text-base font-bold text-gray-900 dark:text-gray-100">Personal Information</h2>
                  <p className="text-xs text-gray-500 mt-0.5 dark:text-gray-400">Manage your personal profile details</p>
                </div>

                {!editing ? (
                  <button
                    onClick={() => setEditing(true)}
                    className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold transition-colors border border-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 dark:border-gray-700"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">First Name</label>
                  {editing
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">Last Name</label>
                  {editing
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">Email Address</label>
                  <DisplayField icon={<Mail className="w-3.5 h-3.5" />} value={email} />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">Phone Number</label>
                  {editing
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">Date of Birth</label>
                  {editing
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 dark:text-gray-400">Gender</label>
                  {editing ? (
              ...
              <div className="pt-4 border-t border-gray-200 flex items-center gap-2.5 text-xs text-gray-500 dark:border-gray-700 dark:text-gray-400">
          ...
          {activeTab === 'tickets' && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 sm:p-8 space-y-6 dark:bg-gray-900 dark:border-gray-800">
              <div className="border-b border-gray-200 pb-4 dark:border-gray-700">
                <h2 className="text-base font-bold text-gray-900 dark:text-gray-100">My Cinema Tickets</h2>
                <p className="text-xs text-gray-500 mt-0.5 dark:text-gray-400">Your ticket history and active reservations</p>
              </div>

              {bookingsLoading ? (
                <div className="space-y-3">
                  {[1,2,3].map((n) => (
                    <div key={n} className="p-4 bg-white rounded-xl border border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="space-y-3">
                  {bookings.map((b) => (
                    <Link to={`/ticket-confirmation/${b.id}`} key={b.id}
                      className="p-4 bg-white rounded-xl border border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-[#00a8cc] hover:shadow-sm transition-all group dark:bg-gray-800 dark:border-gray-700">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2.5">
                          <h3 className="font-semibold text-gray-900 text-sm group-hover:text-[#00a8cc] transition-colors dark:text-gray-100">{b.showtime?.movie?.title || 'Movie'}</h3>
                      ...
                      <div className="text-right shrink-0">
                        <span className="block text-[10px] text-gray-400 font-mono dark:text-gray-500">REF #{b.id.slice(-6).toUpperCase()}</span>
                ...
                <div className="text-center py-12">
                  <Ticket className="w-8 h-8 text-gray-300 mx-auto mb-2 dark:text-gray-600" />
                  <p className="text-sm text-gray-400 dark:text-gray-500">No ticket records found.</p>
                </div>
              )}
            </div>
          )}
