...
// ─── Seat type config ─────────────────────────────────────────────────────────
const TYPES = {
  STANDARD: { label: 'Standard', bg: 'bg-gray-100',        border: 'border-gray-300',        text: 'text-gray-600',   selBg: 'bg-[#00a8cc]',   selBorder: 'border-[#00a8cc]',   selText: 'text-white',  dot: 'bg-[#00a8cc]' },
  PREMIUM:  { label: 'Premium',  bg: 'bg-amber-50',         border: 'border-amber-300',        text: 'text-amber-700',  selBg: 'bg-amber-500',   selBorder: 'border-amber-400',   selText: 'text-white',  dot: 'bg-amber-400' },
  RECLINER: { label: 'Recliner', bg: 'bg-purple-50',        border: 'border-purple-300',       text: 'text-purple-700', selBg: 'bg-purple-500',  selBorder: 'border-purple-400',  selText: 'text-white',  dot: 'bg-purple-400' },
} as const;
  ...
  <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div className="bg-white border border-gray-200 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-sm font-bold text-gray-900">Delete Row {row}?</h3>
          <p className="text-xs text-gray-600 mt-1">
            This will permanently remove <span className="text-gray-900 font-bold">{count} seat{count !== 1 ? 's' : ''}</span> from this hall.
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={onCancel}
          className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-xs rounded-xl transition-colors"
        >
          Cancel
        </button>
      </div>
...
export const AdminSeatsPage: React.FC = () => {
  ...
  if (hallsLoading) return (
    <div className="space-y-4 max-w-5xl mx-auto">
      ...
      <div className="flex gap-3">{[1,2,3].map(n => <div key={n} className="h-10 w-40 shimmer rounded-xl" />)}</div>
    </div>
  );
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      {!hallId && (
        <div className="bg-white border border-gray-200 rounded-2xl py-16 text-center">
          ...
          <p className="text-sm font-semibold text-gray-600">Select a hall above to edit its seat map</p>
        </div>
      )}
          ...
          {['A','B','C','D','E','F'].map(r => (
            <div key={r} className="flex gap-1.5 justify-center">
              {Array.from({length: 10}).map((_, i) => (
                <div key={i} className="w-8 h-8 shimmer rounded" />
              ))}
            </div>
          ))}
        </div>
      ...
      {hallId && !seatsLoading && (
        <div className="space-y-5">
          ...
          {/* Hall info strip */}
          {hall && (
            <div className="flex flex-wrap gap-4 text-xs text-gray-600 bg-white border border-gray-200 rounded-xl px-4 py-3 items-center">
              ...
              <span className="text-gray-400">·</span>
              <span>Capacity: <span className="text-gray-900 font-bold">{hall.capacity}</span></span>
              <span className="ml-auto flex gap-3">
                {(Object.keys(TYPES) as SeatType[]).map(t => (
                  <button key={t} onClick={() => selectByType(t)}
                    ...
                    {TYPES[t].label}: <span className="text-gray-900 font-bold ml-0.5">{stats[t]}</span>
                  </button>
                ))}
              </span>
            </div>
          ...
          {/* Toolbar */}
          <div className={`bg-white border rounded-xl px-4 py-3 flex flex-wrap items-center gap-3 transition-all ${selected.size > 0 ? 'border-[#00a8cc]/40' : 'border-gray-200'}`}>
            <span className="text-xs text-gray-600">
              <span className="font-black text-gray-900">{selected.size}</span> seat{selected.size !== 1 ? 's' : ''} selected
            </span>

            <div className="flex gap-1.5 ml-1">
              <button onClick={selectAll} className="px-2.5 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-900 text-[11px] font-bold rounded transition-colors">All</button>
              <button onClick={clearAll}  className="px-2.5 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-900 text-[11px] font-bold rounded transition-colors">None</button>
            </div>

            {selected.size > 0 && (
              <>
                <div className="w-px h-5 bg-gray-200 mx-1" />
                <span className="text-xs text-gray-500 font-semibold">Change to:</span>
          ...
          {seats.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-2xl py-12 text-center">
              <p className="text-sm text-gray-500">No seats yet. Add rows below.</p>
            </div>
          ) : (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5 overflow-x-auto">
                  ...
                  return (
                    <div key={row} className="flex items-center gap-3 justify-center w-full">
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-rose-500 transition-colors shrink-0 rounded hover:bg-rose-500/10"
                      >
              ...
              {/* Legend */}
              <div className="flex flex-wrap gap-5 text-[11px] text-gray-600 pt-4 border-t border-gray-200">
                {(Object.keys(TYPES) as SeatType[]).map(t => (
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex flex-wrap items-end gap-4">
              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wide">Row Letter</label>
                <input
                  value={newRow}
                  onChange={e => setNewRow(e.target.value)}
                  placeholder={rows.length > 0 ? String.fromCharCode(rows[rows.length - 1].charCodeAt(0) + 1) : 'A'}
                  maxLength={1}
                  className="w-14 bg-gray-50 border border-gray-300 focus:border-[#00a8cc] text-gray-900 text-sm font-black text-center rounded-lg px-3 py-2.5 focus:outline-none uppercase transition-colors"
                />
              </div>
              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wide">Seat Count</label>
                <input
                  type="number"
                  value={newCount}
                  onChange={e => setNewCount(Math.max(1, Math.min(30, Number(e.target.value))))}
                  min={1} max={30}
                  className="w-20 bg-gray-50 border border-gray-300 focus:border-[#00a8cc] text-gray-900 text-xs rounded-lg px-3 py-2.5 focus:outline-none transition-colors"
                />
              </div>
              <div className="space-y-1.5">
                <label className="block text-[11px] font-bold text-gray-600 uppercase tracking-wide">Seat Type</label>
                <div className="flex gap-1.5">
                  {(Object.keys(TYPES) as SeatType[]).map(t => (
                    <button
                      key={t}
