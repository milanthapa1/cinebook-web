  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Showtimes</h1>
          <p className="text-sm text-gray-500 mt-0.5 dark:text-gray-400">Schedule screenings per hall · <span className="font-semibold text-gray-700 dark:text-gray-300">{showtimes?.length ?? 0}</span> scheduled</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          {movieFilter && (
            <button
              ...
              className="px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold rounded-xl border border-rose-200 transition-colors dark:bg-rose-900/40 dark:hover:bg-rose-900/60 dark:text-rose-300 dark:border-rose-800">
              Bulk Delete
            </button>
          )}
          <button
      ...
      {error && <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-600 dark:bg-rose-900/40 dark:border-rose-800 dark:text-rose-300">{error}</div>}
          ...
          {Array.from({ length: 3 }).map((_, n) => (
            <div key={n} className="flex items-center px-5 py-3.5 border-b border-gray-50 dark:border-gray-800 last:border-0">
              <div className="flex-1 flex items-center gap-2.5">
                <div className="skeleton w-6 h-9 rounded-lg shrink-0" />
                <div className="skeleton h-2.5 w-28 rounded" />
              </div>
              <div className="flex-1"><div className="skeleton h-5 w-20 rounded-lg" /></div>
              <div className="flex-1"><div className="skeleton h-2.5 w-28 rounded" /></div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm dark:bg-gray-900 dark:border-gray-800">
          <div className="overflow-x-auto">
            <table className="w-full text-xs min-w-[640px]">
              ...
              <tbody>
                {showtimes?.map((s, i) => (
                  <tr key={s.id} className={`hover:bg-[#00a8cc]/5 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-800 ${i % 2 === 1 ? 'bg-gray-50/30 dark:bg-gray-800/30' : ''}`}>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <img src={s.movie.posterUrl} alt={s.movie.title} className="w-6 h-9 rounded-lg object-cover border border-gray-200 shrink-0 dark:border-gray-700" />
                        <span className="font-bold text-gray-900 truncate max-w-[130px] dark:text-gray-100">{s.movie.title}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded-lg font-semibold dark:bg-gray-800 dark:text-gray-400">{s.hall.name}</span>
                    </td>
                    <td className="px-5 py-3.5 text-gray-600 whitespace-nowrap font-medium dark:text-gray-400">{fmt(s.startsAt)}</td>
                    <td className="px-5 py-3.5">
                      <span className="font-bold text-gray-900 dark:text-gray-100">NPR {s.basePrice}</span>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1.5 justify-end">
                        <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-500 transition-colors dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-400" title="Edit">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => setConfirmDel(s.id)} className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-500 transition-colors dark:bg-rose-900/40 dark:hover:bg-rose-900/60 dark:text-rose-300" title="Delete">
          ...
          {!showtimes?.length && (
            <div className="py-16 text-center">
              ...
              <p className="text-sm font-semibold text-gray-400 dark:text-gray-500">No showtimes scheduled</p>
              <p className="text-xs text-gray-400 mt-1 dark:text-gray-500">Add your first showtime to get started</p>
            </div>
          )}
        </div>
      )}
      ...
      {confirmDel && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl dark:bg-gray-900 dark:border-gray-800">
            <div className="w-10 h-10 bg-rose-50 rounded-xl flex items-center justify-center dark:bg-rose-900/40">
              <Trash2 className="w-5 h-5 text-rose-500" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100">Delete Showtime?</h3>
              <p className="text-xs text-gray-500 mt-1 dark:text-gray-400">Existing bookings for this showtime will be affected. This cannot be undone.</p>
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={() => setConfirmDel(null)} className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-xs rounded-xl transition-colors dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200">Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-md shadow-2xl dark:bg-gray-900 dark:border-gray-800">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              ...
              <button onClick={() => setShowForm(false)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 transition-colors dark:hover:bg-gray-800"><X className="w-4 h-4" /></button>
            </div>
